...
from ..agents.types import AgentAudio, AgentImage, handle_agent_input_types, handle_agent_output_types
from .validation import MethodChecker, validate_tool_attributes  # tools/validation.py 是 tool_validation 的新位置
...
from ..base.tool import Tool as AbstractTool
...
class Tool(BaseTool):
    """
    智能体使用的工具基类。继承自 `smart_agent.base.tool.Tool`。
    ...
    def to_dict(self) -> dict:
        ...
        requirements = {el for el in get_imports(tool_code) if el not in sys.stdlib_module_names} | {"smart_agent"}
    ...
    def _initialize_hub_repo(repo_id: str, token: bool | str | None, private: bool | None) -> str:
        ...
        metadata_update(repo_url.repo_id, {"tags": ["smart_agent", "tool"]}, repo_type="space", token=token)
    ...
    def _get_gradio_app_code(self, tool_module_name: str = "tool") -> str:
        ...
        return textwrap.dedent(
            f"""\
            from smart_agent import launch_gradio_demo
...
class ToolCollection:
    ...
    def from_hub(
        ...
        Example:
        ```py
        >>> from smart_agent import ToolCollection, CodeAgent
    ...
    def from_mcp(
        ...
        ```py
        >>> import os
        >>> from smart_agent import ToolCollection, CodeAgent, InferenceClientModel
        ...
        try:
            ...
            raise ImportError(
                """Please install 'mcp' extra to use ToolCollection.from_mcp: `pip install 'smart_agent[mcp]'`."""
            )
        ...
        **hub_kwargs,
        ...
        if not _is_package_available("accelerate") or not _is_package_available("torch"):
            raise ModuleNotFoundError(
                "Please install 'transformers' extra to use a PipelineTool: `pip install 'smart_agent[transformers]'`"
            )
...
def get_tools_definition_code(tools: dict[str, Tool]) -> str:
    ...
    for tool in tools.values():
        ...
        tool_code = tool_code.replace("from .core_tools import Tool", "")
