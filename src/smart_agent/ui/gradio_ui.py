...
from smart_agent.agents.types import AgentAudio, AgentImage, AgentText
from smart_agent.agents.core import MultiStepAgent, PlanningStep
from ..memory.core import ActionStep, FinalAnswerStep

from smart_agent.llm.models import ChatMessageStreamDelta, MessageRole, agglomerate_stream_deltas
from smart_agent.utils.general import _is_package_available
...
def pull_messages_from_step(step_log: ActionStep | PlanningStep | FinalAnswerStep, skip_model_outputs: bool = False):
    ...
    if not _is_package_available("gradio"):
        raise ModuleNotFoundError(
            "Please install 'gradio' extra to use the GradioUI: `pip install 'smart_agent[gradio]'`"
        )
...
class GradioUI:
    ...
    This class requires the `gradio` extra to be installed: `pip install 'smart_agent[gradio]'`.
    ...
    Example:
        ```python
        from smart_agent import CodeAgent, GradioUI, InferenceClientModel
