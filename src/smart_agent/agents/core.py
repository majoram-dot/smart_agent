...
from ..tools.default import TOOL_MAPPING, FinalAnswerTool
from ..executors.local_python import BASE_BUILTIN_MODULES, LocalPythonExecutor, PythonExecutor, fix_final_answer_code
from ..memory.core import ActionStep, AgentMemory, CallbackRegistry, FinalAnswerStep, MemoryStep, PlanningStep, SystemPromptStep, TaskStep, Timing, ToolCall
...
from ..executors.remote import BlaxelExecutor, DockerExecutor, E2BExecutor, ModalExecutor, WasmExecutor
from ..tools.core import Tool, validate_tool_arguments
from ..tools import core as tools_module
...
class MultiStepAgent(ABC):
    ...
    多步骤Agent基类，采用ReAct框架逐步解决任务：
    - Action（由LLM生成的行动）
    - Observation（从环境获取的观察结果）
    循环执行直到任务完成。

    Args:
        tools (`list[Tool]`): [`Tool`]s that the agent can use. Agent可使用的工具列表
        model (`Callable[[list[dict[str, str]]], ChatMessage]`): Model that will generate the agent's actions. 生成Agent行动的模型
        prompt_templates ([`~agents.PromptTemplates`], *optional*): Prompt templates. 提示词模板
        instructions (`str`, *optional*): Custom instructions for the agent, will be inserted in the system prompt. 自定义指令
        max_steps (`int`, default `20`): Maximum number of steps the agent can take to solve the task. 最大步数限制
        add_base_tools (`bool`, default `False`): Whether to add the base tools to the agent's tools. 是否添加工具
        verbosity_level (`LogLevel`, default `LogLevel.INFO`): Level of verbosity of the agent's logs. 日志详细程度
        managed_agents (`list`, *optional*): Managed agents that the agent can call. 可管理的子Agent
        step_callbacks (`list[Callable]` | `dict[Type[MemoryStep], Callable | list[Callable]]`, *optional*): Callbacks that will be called at each step. 每步回调函数
        planning_interval (`int`, *optional*): Interval at which the agent will run a planning step. 规划间隔
        name (`str`, *optional*): Necessary for a managed agent only - the name by which this agent can be called. Agent名称
        description (`str`, *optional*): Necessary for a managed agent only - the description of this agent. Agent描述
        provide_run_summary (`bool`, *optional*): Whether to provide a run summary when called as a managed agent. 是否提供运行摘要
        final_answer_checks (`list[Callable]`, *optional*): List of validation functions to run before accepting a final answer. 最终答案校验函数
        return_full_result (`bool`, default `False`): Whether to return the full [`RunResult`] object or just the final answer output from the agent run. 是否返回完整结果
    """
    ...
    def __init__(
        ...
        self.agent_name = self.__class__.__name__  # Agent类名
        self.model = model  # 大语言模型
        self.prompt_templates = prompt_templates or EMPTY_PROMPT_TEMPLATES  # 提示词模板
        ...
        self.step_number = 0  # 当前步数
        self.planning_interval = planning_interval  # 规划间隔
        self.state: dict[str, Any] = {}  # Agent状态存储
        self.name = self._validate_name(name)  # 验证名称合法性
        self.description = description  # Agent描述
        self.provide_run_summary = provide_run_summary  # 是否提供运行摘要
        self.final_answer_checks = final_answer_checks if final_answer_checks is not None else []  # 最终答案校验
        self.return_full_result = return_full_result  # 是否返回完整结果
        self.instructions = instructions  # 自定义指令
        self._setup_managed_agents(managed_agents)  # 设置管理的子Agent
        self._setup_tools(tools, add_base_tools)  # 设置工具列表
        self._validate_tools_and_managed_agents(tools, managed_agents)  # 验证工具和Agent名称唯一性

        self.task: str | None = None  # 当前任务
        self.memory = AgentMemory(self.system_prompt)  # Agent记忆系统

        if logger is None:
            self.logger = AgentLogger(level=verbosity_level)  # 创建日志器
        else:
            self.logger = logger

        self.monitor = Monitor(self.model, self.logger)  # 创建监控器
        self._setup_step_callbacks(step_callbacks)  # 设置步骤回调
    ...
    def _setup_managed_agents(self, managed_agents: list | None = None) -> None:
        ...
        设置被管理的子Agent，将其注册为可被主Agent调用的工具。
        """
        self.managed_agents = {}
        if managed_agents:
    ...
    def _setup_tools(self, tools, add_base_tools):
        ...
        将工具列表转换为字典，以名称为key便于快速查找。
        如果add_base_tools为True，则添加基础工具（如文件操作、网络搜索等）。
        """
        ...
        self.tools.setdefault("final_answer", FinalAnswerTool())  # 默认添加最终答案工具

    def _validate_tools_and_managed_agents(self, tools, managed_agents):
        """验证工具和管理的Agent名称是否唯一，避免冲突。"""
...
{str(additional_args)}."""
    ...
    def from_dict(cls, agent_dict: dict[str, Any], **kwargs) -> "MultiStepAgent":
        ...
        for tool_info in agent_dict["tools"]:
            tools.append(tools_module.Tool.from_code(tool_info["code"]))
        # Load managed agents
        managed_agents = []
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("smart_agent.prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._use_structured_outputs_internally:
            prompt_templates = prompt_templates or yaml.safe_load(
                importlib.resources.files("smart_agent.prompts").joinpath("structured_code_agent.yaml").read_text()
            )
        else:
            prompt_templates = prompt_templates or yaml.safe_load(
                importlib.resources.files("smart_agent.prompts").joinpath("code_agent.yaml").read_text()
            )
