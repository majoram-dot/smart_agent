...
from ..monitoring.core import TokenUsage
from ..tools.core import Tool
from ..utils.general import RateLimiter, Retrying, _is_package_available, encode_image_base64, make_image_url, parse_json_blob
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not _is_package_available("vllm"):
            raise ModuleNotFoundError("Please install 'vllm' extra to use VLLMModel: `pip install 'smart_agent[vllm]'`")
...
class MLXModel(Model):
    ...
    > [!TIP]
    > You must have `mlx-lm` installed on your machine. Please run `pip install 'smart_agent[mlx-lm]'` if it's not the case.
        ...
        **kwargs,
        ...
        if not _is_package_available("mlx_lm"):
            raise ModuleNotFoundError(
                "Please install 'mlx-lm' extra to use 'MLXModel': `pip install 'smart_agent[mlx-lm]'`"
            )
        import mlx_lm
...
class TransformersModel(Model):
    ...
    > [!TIP]
    > You must have `transformers` and `torch` installed on your machine. Please run `pip install 'smart_agent[transformers]'` if it's not the case.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'transformers' extra to use 'TransformersModel': `pip install 'smart_agent[transformers]'`"
            )
...
class LiteLLMModel(ApiModel):
    ...
    def create_client(self):
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'litellm' extra to use LiteLLMModel: `pip install 'smart_agent[litellm]'`"
            ) from e
...
class LiteLLMRouterModel(LiteLLMModel):
    ...
    ```python
    >>> import os
    >>> from smart_agent import CodeAgent, WebSearchTool, LiteLLMRouterModel
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'litellm' extra to use LiteLLMRouterModel: `pip install 'smart_agent[litellm]'`"
            ) from e
...
class OpenAIModel(ApiModel):
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'openai' extra to use OpenAIModel: `pip install 'smart_agent[openai]'`"
            ) from e
...
class AzureOpenAIModel(OpenAIModel):
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'openai' extra to use AzureOpenAIModel: `pip install 'smart_agent[openai]'`"
            ) from e
...
class AmazonBedrockModel(ApiModel):
    ...
    def create_client(self):
        try:
            ...
            raise ModuleNotFoundError(
                "Please install 'bedrock' extra to use AmazonBedrockServerModel: `pip install 'smart_agent[bedrock]'`"
            ) from e
