...
from smart_agent.llm.models import ChatMessage, MessageRole, get_dict_from_nested_dataclasses
from ..monitoring.core import AgentLogger, LogLevel, Timing, TokenUsage
from smart_agent.utils.general import AgentError, make_json_serializable


if TYPE_CHECKING:
    import PIL.Image

    from smart_agent.llm.models import ChatMessage
    from ..monitoring.core import AgentLogger
...
@dataclass
class MemoryStep:
    """记忆步骤的基类，抽象出所有步骤的通用接口。"""
    
    def dict(self):
        return asdict(self)

    def to_messages(self, summary_mode: bool = False) -> list[ChatMessage]:
        """将记忆步骤转换为聊天消息列表，用于构建prompt。"""
...
class ActionStep(MemoryStep):
    ...
    timing: Timing  # 计时信息
    model_input_messages: list[ChatMessage] | None = None  # 发送给模型的输入消息
    ...
    code_action: str | None = None  # 代码动作
    ...
    def to_messages(self, summary_mode: bool = False) -> list[ChatMessage]:
        ...
        Args:
            summary_mode: 是否为摘要模式，摘要模式下跳过详细输出
        """
        messages = []
